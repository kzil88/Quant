...
def buy(stock_code,opdate,buy_money):
    ...
    if deal_buy.cur_money_rest > buy_money:
        sql_buy = "select * from stock_all a where a.state_dt = '%s' and a.stock_code = '%s'" % (
        opdate, stock_code)
...
def sell(stock_code,opdate,predict):
    ...
    sql_sell_select = "select * from stock_all a where a.state_dt = '%s' and a.stock_code = '%s'"%(opdate,stock_code)
