...
def buy(stock_code,opdate,buy_money,slayer):
    # 建立数据库连接
    db = pymysql.connect(host='127.0.0.1', user='root', passwd='admin', db='stock', charset='utf8')
    cursor = db.cursor()
    deal_buy = Deal.Deal()
    #后买入
    if deal_buy.cur_money_rest > buy_money:
        sql_buy = "select * from stock_price_day_list a where a.state_dt = '%s' and a.stock_code = '%s'" % (
        opdate, stock_code)
        ...
        if buy_price >= 195:
            return 0
        vol, rest = divmod(min(deal_buy.cur_money_rest, 20000), buy_price * 100)
        vol = vol * 100
        ...
        sql_buy_update2 = "insert into my_capital(capital,money_lock,money_rest,deal_action,stock_code,stock_vol,state_dt,deal_price,score)VALUES ('%.2f', '%.2f', '%.2f','%s','%s','%i','%s','%.2f','%i')" % (new_capital, new_money_lock,new_money_rest, 'buy', stock_code, vol, opdate, buy_price,slayer)
...
def sell(stock_code,opdate):
    ...
    sql_sell_select = "select * from stock_price_day_list a where a.state_dt = '%s' and a.stock_code = '%s'"%(opdate,stock_code)
    ...
    elif hold_days >= 5 and hold_vol > 0:
        new_money_lock = deal.cur_money_lock - init_price * hold_vol
        new_money_rest = deal.cur_money_rest + sell_price * hold_vol
        new_capital = deal.cur_capital + (sell_price - init_price) * hold_vol
        new_profit = (sell_price - init_price) * hold_vol
