...
def ChooseMain(index_date):
    ...
    for i in range(len(date_seq)-4):
        ...
        # 策略六: 日涨幅超3% 以换手率和量比做逆排序
        sql_select_s6 = "select * from stock_price_day_list a where a.ro > 1.03 and a.state_dt = '%s'" % (date_seq[i])
        ans = q3_ana(sql_select_s6, 6, date_seq[i], date_seq[i + 4])
    db.close()
    db = pymysql.connect(host='127.0.0.1', user='root', passwd='admin', db='stock', charset='utf8')
    cursor = db.cursor()
    ...
    for k in range(len(done_set_group)):
        ...
        elif done_set_group[k][0] == 6:
            score6 = score6 + float(done_set_group[k][1])*0.4
    for j in range(len(done_set_group2)):
        ...
        elif done_set_group2[j][0] == 6:
            score6 = score6 + float(done_set_group2[j][1])*0.6
    score_merge = [score1,score2,score3,score4,score5,score6]
