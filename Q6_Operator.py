...
def buy(stock_code,opdate,buy_money,year):
    ...
    if deal_buy.cur_money_rest > buy_money:
        sql_buy = "select * from stock_%s a where a.state_dt = '%s' and a.stock_code = '%s'" % (str(year),
        opdate, stock_code)
...
def sell(stock_code,opdate,predict,year):
    # 建立数据库连接
    db = pymysql.connect(host='127.0.0.1', user='root', passwd='admin', db='stock', charset='utf8')
    cursor = db.cursor()
    ...
    sql_sell_select = "select * from stock_%s a where a.state_dt = '%s' and a.stock_code = '%s'"%(str(year),opdate,stock_code)
