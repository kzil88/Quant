...
#             self.avg = ta.MA(np.array(self.close_list), period)
...
#                 self.cnt_risk[a] = len([x for x in self.close_list[:a + period - 1] if x <= self.close_list[a + period - 1]])
#             # ARFQ
...
#         self.open_list.append(float(resu))
#         self.close_list.append(float(resu))
#         self.high_list.append(float(resu)+float(new_high))
#         self.low_list.append(float(resu)-float(new_low))
#         self.vol_list.append(float(self.vol_list[-1]*0.9))
#         self.tor_list.append(float(self.tor_list[-1]*0.9))
#         self.vr_list.append(float(self.vr_list[-1]*0.9))
#         period = min(20, len(self.close_list))
#         self.avg = ta.MA(np.array(self.close_list),period)
...
#             train = [self.avg[len(self.avg)-1-i],self.vol_list[len(self.vol_list)-1-i],self.cnt_bad_sell[len(self.cnt_bad_sell)-1-i],self.cnt_good_buy[len(self.cnt_good_buy)-1-i],self.cnt_good_sell[len(self.cnt_good_sell)-1-i],self.cnt_risk[len(self.cnt_risk)-1-i],self.af[len(self.af)-1-i],self.fq[len(self.fq)-1-i],self.macd_list[len(self.macd_list)-1-i],self.bool_up[len(self.bool_up)-1-i],self.bool_mid[len(self.bool_mid)-1-i],self.bool_dn[len(self.bool_dn)-1-i],self.tor_list[len(self.tor_list)-1-i],self.vr_list[len(self.vr_list)-1-i]]
#             self.data_train.append(np.array(train))
#             self.data_target.append(self.close_list[len(self.close_list) - i])
#             #self.data_target.append(self.avg[len(self.avg)-i])
#         self.test_case = np.array([self.avg[-1],self.vol_list[-1],self.cnt_bad_sell[-1],self.cnt_good_buy[-1],self.cnt_good_sell[-1],self.cnt_risk[-1],self.af[-1],self.fq[-1],self.macd_list[-1],self.bool_up[-1],self.bool_mid[-1],self.bool_dn[-1],self.tor_list[-1],self.vr_list[-1]])
