...
if __name__ == '__main__':
    ...
    #temp_index = ts.get_hist_data(code= 'sh',start='2015-03-10',end=end_dt)

    temp_index = ts.bar(code='000001', conn = cons,freq='D',start_date='2015-01-01',end_date=end_dt,ma=[5,10,20,30,60],asset='INDEX')
    print(temp_index)
    ...
    for j in range(c_len):
        ...
        try:
            sql_insert = "INSERT INTO stock_index(state_dt,stock_code,open,close,high,low,vol,amount,ma5,ma10,ma20,ma30,ma60,ro,p_change) VALUES ('%s', '%s', '%.2f', '%.2f','%.2f','%.2f','%i','%.2f','%.2f','%.2f','%.2f','%.2f','%.2f','%.4f','%.2f')" % (str(state_dt),str('SH'),float(resu[1]),float(resu[2]),float(resu[3]),float(resu[4]),int(resu[5]),float(resu[6]),float(resu[7]),float(resu[8]),float(resu[9]),float(resu[10]),float(resu[11]),float(ro),float(p_change))
